...
// Fonctions globales pour simplifier l'usage dans les vues
...
{
    return UrlHelper::assetUrl($path);
}

function api_url(string $endpoint): string
{
    return UrlHelper::apiUrl($endpoint);
}

function redirect_to(string $path, int $statusCode = 302): never
{
    UrlHelper::redirectTo($path, $statusCode);
}

function versioned_asset(string $path): string
{
    return UrlHelper::versionedAsset($path);
}
